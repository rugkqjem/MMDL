...
- 2026-09-25 re-run with 32768 / 128000 (`outputs/qwen3vl4b_mmmu_val_32k/`): Qwen 62.00 / MMMU 62.78, 98 truncated, 8666.7 s generation vs 2524.6 s. Only 1/900 response finished beyond 16384; 97/98 truncations are repetition loops (zlib ratio of the last 10k chars < 0.10), so a bigger budget does not fix truncation. Only 132/900 responses are identical to the 16384 run despite the same seed; per-question correctness agrees on 813/900 (43 wrong→right, 44 right→wrong, McNemar p = 1.0). Defaults stay 16384 / 32768.
- 2026-09-26 re-run with the exact first-run config (`outputs/qwen3vl4b_mmmu_val_rerun/`): Qwen 63.56 / MMMU 62.67, 78 truncated, 2437.8 s. Only 145/900 responses identical to the first run (responses diverge within the first few hundred tokens), correctness agrees on 807/900 (53 / 40, McNemar p = 0.21) — so run-to-run variation is inherent (vLLM dynamic batching is not batch-invariant), not caused by the max-token change. A single run can move ~1.5 points.
- **Reporting rule (team decision): report and compare single-run results and their responses only — never average multiple runs.** The official baseline is the first run (62.11); the 32k and rerun folders are reference material.
