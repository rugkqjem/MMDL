# Qwen3-VL-4B MMMU-val 베이스라인 정리 (2026-09-23, 09-25 보완)
...
- 결과 파일: `outputs/qwen3vl4b_mmmu_val/` (`predictions.jsonl` 원본 응답, `predictions_meta.json`, `scores_*`). 32768 재실행 결과는 `outputs/qwen3vl4b_mmmu_val_32k/`
...
- 900개 중 132개만 응답이 완전히 같음. `max_model_len`이 바뀌면 vLLM의 배치 구성과 수치가 달라져 생성 경로가 갈라지는 것으로 추정 (요청별 seed는 샘플링 난수만 고정)
...
4. **샘플링 변동**: temperature 0.7이라 실행마다 결과가 달라짐. 표준오차 약 ±1.6점. 09-25 재실행과 비교하면 문제의 약 10%(87개)가 정답 여부가 바뀜 (총점 차이 0.1점은 우연히 상쇄된 것)
...
- **반복 실행**: 한 번 실행하면 문제의 약 10%가 바뀜 (32768 재실행으로 확인). fine-tuning 전후 비교를 여러 번 실행한 평균으로 할지, 문제별로 짝지은 비교(McNemar)로 할지 (1회 약 45분)
